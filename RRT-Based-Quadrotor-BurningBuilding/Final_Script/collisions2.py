...
class PolygonEnvironment:
    ...
    def read_env(self, env_file_path):
        ...
        line_count = 0
        column_count = 0
        self.row_num = len(file_infos)
        self.col_num = len(file_infos[0])-1
        self.x_min = -float(self.col_num)/2/50
        self.x_max = float(self.col_num)/2/50
        self.y_min = -float(self.row_num)/2/50
        self.y_max = float(self.row_num)/2/50
        self.lims = np.array([[self.x_min, self.x_max], [self.y_min, self.y_max]])
        print 'Map readed: X[', self.x_min, ', ', self.x_max, ']  Y[', self.y_min, ',', self.y_max, '].'
        for line in file_infos:
            column_count = 0
            for element in line:
                if element == 's':
                    self.start = np.array([float(2*column_count - self.col_num)/2/50, -float(2*line_count - self.row_num)/2/50])
                if element == 'g':
                    self.goal = np.array([float(2*column_count - self.col_num)/2/50, -float(2*line_count - self.row_num)/2/50])
                if element == 'w' or element == 'o':
                    self.obstacle.append([float(2*column_count - self.col_num)/2/50, -float(2*line_count - self.row_num)/2/50])
                if element == 'l':
                    self.danger_low.append([float(2*column_count - self.col_num)/2/50, -float(2*line_count - self.row_num)/2/50])
                if element == 'm':
                    self.danger_medium.append([float(2*column_count - self.col_num)/2/50, -float(2*line_count - self.row_num)/2/50])
                if element == 'h':
                    self.danger_high.append([float(2*column_count - self.col_num)/2/50, -float(2*line_count - self.row_num)/2/50])
                column_count += 1
            line_count += 1
        if self.start == None or self.goal == None:
            return False
        print 'Start Point is set:', self.start, 'Goal Point is set:', self.goal
        self.parse_rect_robot([0.1, 0.1])
    ...
    def check_cost(self, state):
        # discretization of state
        temp = [int(state[0]*100), int(state[1]*100)]
        for i in range(2):
            if temp[i] % 2 != 0:
                temp[i] += 1
        state = [float(temp[0])/100, float(temp[1])/100]
        if state in self.danger_low:
            return 1.3
        if state in self.danger_medium:
            return 1.4
        if state in self.danger_high:
            return 1.5
        return 1
    ...
    def draw_env(self, q=None, show=True):
        ...
        for point in self.obstacle:
            O_xlist.append(point[0])
            O_ylist.append(point[1])
        plotter.plot(O_xlist, O_ylist, 'bs')
        for point in self.danger_low:
            plotter.plot(point[0],point[1],color='yellow', marker='.', markersize=1)
        for point in self.danger_medium:
            plotter.plot(point[0],point[1],color='darkorange', marker='.', markersize=1)
        for point in self.danger_high:
            plotter.plot(point[0],point[1],color='red', marker='.', markersize=1)
...
# def test(filename):
#     ppp = PolygonEnvironment()
#     ppp.read_env(filename)
#     ppp.draw_env(None, True)
#     return False
#
# test('Map_Highresolution.dat')